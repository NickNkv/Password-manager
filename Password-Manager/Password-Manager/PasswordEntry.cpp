...
//private helpers
PasswordEntry::PasswordEntry(const char* website, const char* username, const char* password, Cipher* cipher, bool alreadyEncrypted) { //dummy bool variable
	//data validation
	if (!website || strlen(website) == 0) {
		throw std::invalid_argument("Website can not be nullptr or empty!");
	}
	...
	this->encryptedPassword = new (std::nothrow) char[strlen(password) + 1];
	if (!this->encryptedPassword) {
		delete[] this->website;
		delete[] this->username;
		throw std::bad_alloc();
	...
	strcpy(this->website, website);
	strcpy(this->username, username);
	strcpy(this->encryptedPassword, password);

	this->cipher = cipher;
}

//static helpers
PasswordEntry* PasswordEntry::createFromEncrypted(const char* website, const char* username, const char* encryptedPassword, Cipher* cipher)
{
	return new PasswordEntry(website, username, encryptedPassword, cipher, true);
}